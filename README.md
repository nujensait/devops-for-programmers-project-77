# Оглавление
1. [О проекте](#о-проекте)
2. [Статус тестов Hexlet](#статус-тестов-hexlet)
3. [Структура проекта](#структура-проекта)
4. [Быстрый старт](#быстрый-старт)
5. [Команды управления](#команды-управления)
   - [WSL Управление](#wsl-управление)
   - [Terraform Команды](#terraform-команды)
   - [Ansible Команды](#ansible-команды)
   - [Комплексные команды](#комплексные-команды)
6. [Безопасность](#безопасность)
7. [Требования](#требования)
8. [Ключи доступа](#ключи-доступа)
9. [Мониторинг с DataDog](#мониторинг-с-datadog)
10. [Мониторинг с Upmon](#мониторинг-с-upmon)
11. [Контакты](#контакты)

-----

## О проекте
- Курс Hexlet: ["DevOps для программистов"](https://lite.al/VpBen)
- Проект №3: Инфраструктура как код (Terraform)

----

## Статус тестов Hexlet
[![Actions Status](https://github.com/nujensait/devops-for-programmers-project-77/actions/workflows/hexlet-check.yml/badge.svg)](https://github.com/nujensait/devops-for-programmers-project-77/actions)

-----

## Структура проекта

```bash
<project_root>/
├── terraform/          # Инфраструктура как код (Terraform)
│   ├── provider.tf     # Конфигурация провайдера Yandex Cloud
│   ├── backend.tf      # Конфигурация удалённого бэкенда (S3)
│   ├── main.tf         # Основные ресурсы (VM, сети и т. д.)
│   └── variables.tf    # Переменные (несекретные)
├── ansible/            # Конфигурация серверов (Ansible)
│   ├── playbook.yml    # Основной плейбук
│   └── vault.yml       # Зашифрованные переменные (Ansible Vault)
└── README.md           # Описание проекта
```

----

## Быстрый старт

```bash
make setup    # Инициализирует проект
make deploy   # Развертывает инфраструктуру
make clean    # Удаляет все ресурсы
```

## Команды управления

### WSL Управление
| Команда                    | Описание                                                           |
|----------------------------|--------------------------------------------------------------------|
| `make wsl-shutdown`        | Полная остановка WSL (аналог перезагрузки)                         |
| `make wsl-update`          | Обновление WSL до последней версии                                 |
| `make wsl-restart-service` | Перезапуск службы LxssManager (требует PowerShell)                 |
| `make wsl-reset-network`   | Сброс сетевых настроек (решает проблемы с подключением)            |
| `make wsl-reinstall`       | Полная переустановка WSL (сбрасывает дистрибутив на чистую версию) |

### Terraform Команды
| Команда           | Описание                                            |
|-------------------|-----------------------------------------------------|
| `make tf-init`    | Инициализация Terraform (загрузка провайдеров)      |
| `make tf-plan`    | Просмотр планируемых изменений инфраструктуры       |
| `make tf-apply`   | Применение изменений (развертывание инфраструктуры) |
| `make tf-destroy` | Уничтожение всей созданной инфраструктуры           |

### Ansible Команды
| Команда                   | Описание                                                         |
|---------------------------|------------------------------------------------------------------|
| `make ansible-vault-edit` | Редактирование зашифрованного файла с секретами (требует пароль) |
| `make ansible-deps`       | Установка зависимостей Ansible (ролей и коллекций)               |
| `make ansible-play`       | Запуск полного Ansible playbook для настройки и деплоя           |
| `make ansible-setup`      | Запуск только задач настройки серверов (тег setup)               |
| `make ansible-deploy`     | Запуск только задач деплоя приложения (тег deploy)               |
| `make ansible-update`     | Запуск только задач обновления пакетов (тег update)              |
| `make ansible-restart`    | Перезапуск приложения (тег restart)                              |

### Комплексные команды
| Команда       | Описание                                                                |
|---------------|-------------------------------------------------------------------------|
| `make setup`  | Инициализирует весь проект (terraform init + установка зависимостей)    |
| `make deploy` | Полный цикл развертывания: создание инфраструктуры + настройка серверов |
| `make all`    | Полный цикл с инициализацией, планированием и применением Terraform     |
| `make clean`  | Полная очистка (удаление всех облачных ресурсов)                        |

## Мониторинг с DataDog

Проект настроен для использования DataDog для мониторинга инфраструктуры и приложения.

### Настройка DataDog

1. Получите API и APP ключи в [DataDog](https://app.datadoghq.com/organization-settings/api-keys)
2. Добавьте ключи в зашифрованный файл `ansible/vault.yml`:
   ```bash
   make edit-vault
   ```
   И добавьте следующие строки:
   ```yaml
   # DataDog API key
   datadog_api_key: "ваш_api_ключ_datadog"
   datadog_app_key: "ваш_app_ключ_datadog"
   datadog_site: "datadoghq.com"
   ```

### Команды для работы с DataDog

| Команда                      | Описание                                                |
|------------------------------|----------------------------------------------------------|
| `make ansible-deps`          | Установка зависимостей Ansible, включая роль DataDog     |
| `make ansible-play --tags monitoring,datadog` | Установка только агента DataDog на серверы |
| `make show-lb-ip`            | Показать IP-адрес балансировщика нагрузки для настройки мониторинга |

### Мониторы DataDog

В проекте настроены следующие мониторы:

1. **App Availability Check** - проверяет доступность приложения на серверах
2. **Load Balancer Availability** - проверяет доступность балансировщика нагрузки

Мониторы создаются автоматически при выполнении `make tf-apply` после добавления ключей DataDog.

## Мониторинг с Upmon

### Настройка Upmon

1. Получите API ключ в [Upmon](https://upmon.io/)
2. Добавьте ключ в зашифрованный файл `ansible/vault.yml`:
   ```bash
   make edit-vault
   ```
   И добавьте следующие строки:
   ```yaml
   # Upmon API key
   upmon_api_key: "ваш_api_ключ_upmon"
   ```

### Команды для работы с Upmon

| Команда                      | Описание                                                |
|------------------------------|----------------------------------------------------------|
| `make ansible-deps`          | Установка зависимостей Ansible, включая роль Upmon     |
| `make ansible-play --tags monitoring,upmon` | Установка только агента Upmon на серверы |
| `make show-lb-ip`            | Показать IP-адрес балансировщика нагрузки для настройки мониторинга |

### Мониторы Upmon

В проекте настроены следующие мониторы:

1. **App Availability Check** - проверяет доступность приложения на серверах
2. **Load Balancer Availability** - проверяет доступность балансировщика нагрузки

Мониторы создаются автоматически при выполнении `make tf-apply` после добавления ключей Upmon.

## Безопасность
Все секретные переменные хранятся в зашифрованном виде с использованием:
- ansible-vault для Ansible
- Переменных окружения для Terraform

⚠ Важно: Никогда не коммитьте следующие файлы в Git:
- terraform.tfvars
- Любые файлы с паролями в открытом виде
- Файлы состояний Terraform (*.tfstate)

## Требования
- WSL2 (для Windows)
- Terraform >= 1.0
- Ansible >= 2.10
- Yandex Cloud CLI (если используется)

--------

## Ключи доступа

- see in my KeePass : "Hexlet Devops PRJ3"

--------

## Контакты

- Автор: Михаил Иконников <mishaikon@gmail.com>

--------